...
The old automatic conversion and historical-snapshot scanning paths have been retired after verification of the live vault. Drive synchronization accepts the linked-file index; encountering an old live file stops with an error and keeps local changes. New vaults start directly in the linked-file format. A new device downloads all parts on its first load. Later loads reuse validated encrypted parts from IndexedDB and fetch only new references. The index includes encrypted content hashes, record ordering, and deleted-trip markers. Missing or invalid parts fail the whole read rather than silently importing an incomplete vault. Local vault storage and exported backups remain self-contained.
